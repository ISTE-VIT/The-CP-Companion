...
node = (struct TrieNode *)malloc(sizeof(struct TrieNode)); if(node!=NULL)
{
int i;
...
{
int level;
int l = strlen(key); int ind;
struct TrieNode *newnode = root; for (level = 0; level < l; level++)
{
ind = chartoind(key[level]); if (!newnode->children[ind])
...
int main()
{
int n1,i1,ch,hi; char name[100]; char keys[100];
char output[][32] = {"Not present in Phonebook", "Present in Phonebook"};
struct TrieNode *root; root = getNode();
do
{
